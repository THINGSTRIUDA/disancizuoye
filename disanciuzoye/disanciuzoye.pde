...
void setup(){
  size(800,800);
  textS= "holle world,holle world";
  font = createFont("AdobeFangsongStd-Regular-24.vlw", fs);
  textFont(font);textSize(fs);textAlign(CENTER,CENTER);
...
void draw(){
  ...
  noFill();stroke(0,0,80);ellipse(0,0,r*2,r*2);
  fill(60,80,80);
  ...
  bPs[0][0]=width/6;bPs[0][1]=height/2*1.1;
  bPs[1][0]=width/6*2.5;bPs[1][1]=height/4*0.05;
  bPs[2][0]=width/6*3.5;bPs[2][1]=height/4*3.95;
  bPs[3][0]=width/6*5;bPs[3][1]=height/2*0.9;
  pushMatrix();
  ...
  popMatrix();
  noFill();bezier(bPs[0][0],bPs[0][1],bPs[1][0],bPs[1][1],bPs[2][0],bPs[2][1],bPs[3][0],bPs[3][1]);
  fill(240,80,80);
...
for(int i=0;i<s.length();i++){
    ...
    if(cw){rotate(angle+PI/2);}else{rotate(angle-PI/2);}
...
void bendingBezierT(String s,float[][] bPs){
 ...
 void bendingBezierD(String s,float[][] bPs,int n){
  char[] textChar = getChars(s);
  if(n<textChar.length){n=textChar.length;}
  ...
  for(int i = 0;i < numT-1;i++){
    ...
    for(float t = tA[i];t<1;t+=p){
      ...
      if(cL>=eachBL){
        ...
        if((eachBL-pL)<=(cL-eachBL)){tA[i+1]=t;}else{tA[i+1]=t+p;}
        break;
      }
    }
  }
